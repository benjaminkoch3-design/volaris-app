...
export const WorkoutDebriefView: React.FC<WorkoutDebriefViewProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-stone-950/95 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 z-50 overflow-y-auto font-sans">
      <div className="bg-stone-900 border border-stone-800 rounded-3xl p-4 sm:p-5 max-w-md w-full space-y-4 shadow-2xl animate-fadeIn my-auto max-h-[92vh] overflow-y-auto custom-scrollbar">
        ...
        <div className="flex justify-between items-start border-b border-stone-800 pb-2.5">
          <div>
            <span className="text-[9px] font-black text-[#CF9A61] uppercase tracking-widest block">
              Débriefing séance
            </span>
            ...
            {targetKm > 0 && (
              <p className="text-[10px] text-stone-400 font-semibold mt-0.5">
                Prévu : <strong className="text-stone-300">{targetKm} km</strong>
              </p>
            )}
          </div>
          <button
        ...
        <div className="flex bg-stone-950 p-0.5 rounded-xl border border-stone-800 gap-1">
          <button
            type="button"
            onClick={() => setActiveTabMode("manual")}
            className={`flex-1 py-1.5 text-[11px] font-bold uppercase rounded-lg transition cursor-pointer ${
              activeTabMode === "manual"
                ? "bg-[#CF9A61] text-stone-950 shadow-sm font-black"
          ...
          <button
            ...
            className={`flex-1 py-1.5 text-[11px] font-bold uppercase rounded-lg transition cursor-pointer flex items-center justify-center gap-1.5 ${
              activeTabMode === "watch"
                ? "bg-stone-800 text-stone-100 shadow-sm border border-stone-700 font-black"
                : "text-stone-400 hover:text-stone-200"
            }`}
          >
            <span>⌚ Montre</span>
        ...
        {/* 1. SAISIE MANUELLE COMPACTE */}
        {activeTabMode === "manual" && (
          <div className="space-y-2.5 animate-fadeIn">
            {/* DISTANCE & TEMPS CÔTE À CÔTE */}
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="bg-stone-950 p-2.5 rounded-xl border border-stone-800 space-y-1">
                ...
                <div className="flex items-center bg-stone-900 border border-stone-800 rounded-lg px-2.5 py-1.5 focus-within:border-[#CF9A61]">
                  <input
                    type="number"
                    step="0.01"
                    min="0"
                    ...
                    placeholder="0.0"
                    className="w-full text-sm font-black font-mono text-stone-100 bg-transparent focus:outline-none"
                  />
                  <span className="text-[10px] font-bold text-stone-500 pl-1">km</span>
                </div>
              </div>
              ...
              <div className="bg-stone-950 p-2.5 rounded-xl border border-stone-800 space-y-1">
                ...
                <div className="flex items-center bg-stone-900 border border-stone-800 rounded-lg px-2.5 py-1.5 focus-within:border-emerald-500">
                  <input
                    type="number"
                    min="0"
                    step="5"
                  ...
                  />
                  <span className="text-[10px] font-bold text-stone-500 pl-1">m</span>
                </div>
              </div>
            </div>

            {/* TEMPS (H / MIN / SEC) & ALLURE LIVE */}
            <div className="bg-stone-950 p-2.5 rounded-xl border border-stone-800 space-y-1.5">
              <div className="flex justify-between items-center px-0.5">
                ...
                {calculatedPace && (
                  <span className="text-[10px] font-mono font-black text-[#CDCF61]">
              ...
              <div className="grid grid-cols-3 gap-1.5 text-center">
                <div className="flex items-center bg-stone-900 border border-stone-800 rounded-lg px-2 py-1 focus-within:border-[#CF9A61]">
                  <input
                    type="number"
                    min="0"
                    max="23"
                    ...
                    placeholder="0"
                    className="w-full text-center text-xs font-black font-mono text-stone-100 bg-transparent focus:outline-none"
                  />
                  <span className="text-[9px] font-bold text-stone-500">h</span>
                </div>
                ...
                <div className="flex items-center bg-stone-900 border border-stone-800 rounded-lg px-2 py-1 focus-within:border-[#CF9A61]">
                  <input
                    ...
                    className="w-full text-center text-xs font-black font-mono text-stone-100 bg-transparent focus:outline-none"
                  />
                  <span className="text-[9px] font-bold text-stone-500">m</span>
                </div>
                ...
                <div className="flex items-center bg-stone-900 border border-stone-800 rounded-lg px-2 py-1 focus-within:border-[#CF9A61]">
                  <input
                    ...
                    className="w-full text-center text-xs font-black font-mono text-stone-100 bg-transparent focus:outline-none"
                  />
                  <span className="text-[9px] font-bold text-stone-500">s</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* 2. IMPORT MONTRE (CENTRÉ ET MOINS GROS) */}
        {activeTabMode === "watch" && (
          <div className="bg-stone-950 p-3 rounded-xl border border-stone-800 space-y-2.5 animate-fadeIn">
            {isActivityImported && importedActivityName ? (
              <div className="bg-stone-900 border border-stone-800 rounded-xl p-2.5 flex items-center justify-between gap-2">
                <div className="overflow-hidden">
                  <span className="text-[8px] font-bold uppercase text-emerald-400 block">
                    ✓ Activité liée
                  </span>
                ...
                <button
                  ...
                  className="text-[10px] font-bold text-[#CDCF61] bg-stone-950 px-2.5 py-1 rounded-lg border border-stone-800 hover:border-stone-700 transition cursor-pointer shrink-0"
                >
                  📊 Analyser
                </button>
              </div>
              ...
              <div className="space-y-2">
                <span className="text-[9px] font-bold uppercase text-stone-400 block text-center">
                  Importer depuis votre montre :
                </span>

                {/* BOUTONS CENTRÉS ET DISCRETS */}
                <div className="flex justify-center items-center gap-2 max-w-xs mx-auto">
                  {connectedApps.garmin && (
                    <button
                      ...
                      className="flex-1 py-1.5 px-2 bg-stone-900 hover:bg-[#007CC3]/20 border border-stone-800 hover:border-[#007CC3]/50 rounded-lg flex items-center justify-center gap-1.5 transition cursor-pointer"
                    >
                      <GarminLogo className="w-3.5 h-3.5" />
                      <span className="text-[10px] font-bold text-stone-200 uppercase">Garmin</span>
                    </button>
                  )}
                  {connectedApps.coros && (
                    <button
                      ...
                      disabled={Boolean(fetchLoading)}
                      className="flex-1 py-1.5 px-2 bg-stone-900 hover:bg-[#F8283B]/20 border border-stone-800 hover:border-[#F8283B]/50 rounded-lg flex items-center justify-center gap-1.5 transition cursor-pointer"
                    >
                      <CorosLogo className="w-3.5 h-3.5" />
                      <span className="text-[10px] font-bold text-stone-200 uppercase">COROS</span>
                    </button>
                  )}
                  {connectedApps.strava && (
                    <button
                      ...
                      disabled={Boolean(fetchLoading)}
                      className="flex-1 py-1.5 px-2 bg-stone-900 hover:bg-[#FC5200]/20 border border-stone-800 hover:border-[#FC5200]/50 rounded-lg flex items-center justify-center gap-1.5 transition cursor-pointer"
                    >
                      <StravaLogo className="w-3.5 h-3.5" />
                      <span className="text-[10px] font-bold text-stone-200 uppercase">Strava</span>
                    </button>
                  )}
                </div>

                {fetchLoading && (
                  <p className="text-[9px] text-center font-bold text-[#CF9A61] animate-pulse">
            ...
            {syncError && (
              <p className="text-[9px] text-rose-400 font-bold text-center">
                {syncError}
              </p>
            )}

            {showActivityPicker && (
              <div className="bg-stone-900 p-2.5 rounded-xl border border-stone-800 space-y-1.5">
                <span className="text-[8.5px] font-bold text-stone-400 uppercase block">
                ...
                <div className="space-y-1 max-h-32 overflow-y-auto custom-scrollbar">
                  {activitiesList.map((act) => (
                    <div
                      ...
                      className="p-1.5 bg-stone-950 hover:bg-stone-800 border border-stone-800 rounded-lg flex items-center justify-between cursor-pointer transition text-xs"
                    >
                      <div>
                        <div className="font-bold text-stone-200 text-[11px]">{act.title}</div>
                        <div className="text-[8.5px] text-stone-400">
                          {act.date} • {act.distanceKm} km • {act.durationMinutes} min
                        </div>
                      </div>
                      <span className="text-[#CF9A61] font-mono font-bold text-[10px]">
                        {act.avgPace} /km ➔
                      </span>
                    </div>
                  ))}
        ...
        <form onSubmit={handleFormSubmit} className="space-y-3 pt-1">
          ...
          <div className="space-y-1">
            ...
            <select
              ...
              className="w-full bg-stone-950 border border-stone-800 rounded-xl px-2.5 py-1.5 text-xs text-stone-200 focus:outline-none focus:border-[#CF9A61] cursor-pointer"
            >
          ...
          <div className="bg-stone-950 p-2.5 rounded-xl border border-stone-800 space-y-1.5">
            <div className="flex justify-between items-center">
              ...
              <span className="text-[11px] font-black" style={{ color: rpeTheme.text }}>
                {completedRpe}/10 • {rpeTheme.label}
              </span>
            </div>

            <div className="relative w-full h-2 rounded-full bg-gradient-to-r from-emerald-500 via-amber-400 via-orange-500 to-red-600">
              <input
                type="range"
                min="1"
                max="10"
              ...
              />
              <div
                style={{ left: `calc(${((completedRpe - 1) / 9) * 100}% - 6px)` }}
          ...
          <div className="space-y-1">
            <label className="block text-[9px] uppercase font-bold text-stone-400">
              Commentaires & Sensations
            </label>
            <textarea
              rows={2}
              value={comment}
              onChange={(e) => setComment(e.target.value)}
              placeholder="Sensations, météo, forme..."
              className="w-full bg-stone-950 border border-stone-800 rounded-xl p-2.5 text-xs text-stone-200 focus:outline-none focus:border-[#CF9A61] resize-none custom-scrollbar"
            />
          </div>

          {/* ACTIONS */}
          <div className="space-y-1.5 pt-1.5 border-t border-stone-800">
            <div className="flex gap-2">
              ...
              <button
                type="submit"
                className="flex-1 py-2.5 bg-[#CF9A61] hover:bg-[#b88652] text-stone-950 font-black text-xs uppercase rounded-xl shadow-md transition cursor-pointer"
              >
                Enregistrer
              </button>
            </div>
            ...
            {isAlreadyDebriefed && (
              <button
                ...
                className="w-full py-1.5 bg-red-950/30 hover:bg-red-900/50 border border-red-800/40 text-red-400 font-bold text-[9.5px] uppercase tracking-wider rounded-lg transition cursor-pointer"
              >
